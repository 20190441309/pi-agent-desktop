...
interface ClaudeSessionImporterOpts {
    claudeRoot?: string;
    piRoot?: string;
}
...
export class ClaudeSessionImporter {
    ...
    constructor(opts: ClaudeSessionImporterOpts = {}) {
        this.claudeRoot = opts.claudeRoot ?? join(homedir(), ".claude", "projects");
                ...
                if (content.length > 0) {
                    pushMessage(
                        ...
                        content,
                        {
                            api: "claude-import",
                            provider: "anthropic",
                            model: String(message.model ?? "claude-sonnet-4"),
                            stopReason: String(message.stop_reason ?? "stop"),
                        },
    ...
    private async readClaudeSession(filePath: string): Promise<ParsedClaudeSession> {
        ...
        return {
            meta: {
                sessionId: String(firstUserEntry.sessionId),
                cwd: String(firstUserEntry.cwd),
    ...
    private assertClaudeSourcePath(filePath: string): void {
    ...
    private getTargetPath(projectPath: string, session: ParsedClaudeSession): string {
    ...
    private safePathToken(path: string): string {
    ...
    private extractPiText(content: unknown[]): string {
        return content
            .map((item) => {
                const record = item as Record<string, unknown> | null;
                return String(record?.text ?? record?.thinking ?? record?.name ?? "");
            })
            .filter(Boolean)
            .join(" ");
    }
