...
export function InstalledAddons(): React.JSX.Element {
  ...
  return (
    <div className="p-4">
      ...
      {loading ? (
        <div className="py-8 text-center text-sm text-[#999]" role="status">加载已安装项目...</div>
      ) : piPackages.length === 0 && skills.length === 0 ? (
        <div className="py-12 text-center text-sm text-[#999]" role="status">
          暂无已安装插件或 skill
        </div>
      ) : (
        <div className="space-y-3">
          {piPackages.map((item) => (
            <div key={item.source} className="flex items-center justify-between gap-3 rounded-lg border border-[#e5e5e5] bg-white px-3 py-2">
              <div className="min-w-0">
                <div className="truncate text-sm font-medium text-[#1a1a1a]">{item.name}</div>
                <div className="truncate font-mono text-xs text-[#999]">{item.source} · {item.scope === "global" ? "全局" : "项目本地"}</div>
              </div>
              <div className="flex shrink-0 items-center gap-2">
                <button
                  ...
                  className="rounded px-3 py-1 text-xs text-[#666] hover:bg-[#f5f5f5] disabled:opacity-50"
                >
          ...
          {skills.map((skill) => (
            <div key={skill.slug} className="flex items-center justify-between gap-3 rounded-lg border border-[#e5e5e5] bg-white px-3 py-2">
              <div className="min-w-0">
                <div className="truncate text-sm font-medium text-[#1a1a1a]">{skill.slug}</div>
                <div className="text-xs text-[#999]">SkillHub · {skill.enabled ? "已启用" : "已禁用"}</div>
              </div>
              <div className="flex shrink-0 items-center gap-2">
                <button
                  type="button"
      ...
      {pendingRemove && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/35">
          <div className="w-[380px] rounded-2xl border border-[#e5e5e5] bg-white p-5 shadow-2xl" role="dialog" aria-modal="true" aria-label="确认卸载">
            <h3 className="text-base font-semibold text-[#1a1a1a]">卸载{pendingRemove.type === "package" ? "插件" : "技能"}</h3>
            <p className="mt-2 text-sm leading-6 text-[#666]">
              确认卸载 <span className="font-mono text-[#1a1a1a]">{pendingRemove.id}</span> 吗？
            </p>
