...
export type ChartSection = Extract<GeneratedUiSectionV2, { kind: "chart" }>;

/** Pure option builder for unit tests and echarts setOption. */
export function buildChartOption(
  ...
  const { textColor, borderColor } = colors;
      ...
      ? [
          {
            ...
            data: section.data.map((row) => ({
              name: String(row[section.xKey] ?? ""),
              value: Number(row[section.series[0]?.key ?? ""] ?? 0),
            })),
          },
        ]
  ...
  return {
    ...
    grid:
      section.chartType === "pie"
        ? undefined
        : { left: 42, right: 18, top: 16, bottom: 42, containLabel: true },
    xAxis:
      section.chartType === "pie"
        ? undefined
        : {
...
export default function GeneratedUiChart({ section }: GeneratedUiChartProps): React.JSX.Element {
  ...
  useEffect(() => {
    ...
    void import("echarts/core").then(async ({ init, use }) => {
      const [
        ...
        { GridComponent, LegendComponent, TooltipComponent, AriaComponent },
        { CanvasRenderer },
      ] = await Promise.all([
      ...
      chart.setOption(buildChartOption(section, { textColor, borderColor }));
      let resizeFrame: number | null = null;
      const panelLayer = root.closest<HTMLElement>("[data-main-panel]");
      const scheduleResize = (): void => {
        if (panelLayer?.dataset.active === "false" || root.clientWidth === 0 || root.clientHeight === 0)
          return;
      ...
      const panelObserver =
        panelLayer && typeof MutationObserver !== "undefined"
  ...
  return (
    <figure
      className="m-0 rounded-[6px] border border-[var(--mm-border)] bg-[var(--mm-bg-main)] p-3"
      aria-label={section.summary}
    >
      <div ref={rootRef} className="h-[280px] w-full" />
      <figcaption className="mt-2 border-t border-[var(--mm-border)] pt-2 text-[11px] leading-4 text-[var(--mm-text-secondary)]">
        {section.summary}
      </figcaption>
    </figure>
  );
