...
export function GitRailControls({ workspacePath, git, diffStats, onRefresh }: GitRailControlsProps): React.JSX.Element {
  ...
  const rowClass = "flex min-h-9 w-full min-w-0 items-center gap-3 rounded-[6px] px-2.5 text-left hover:bg-[var(--right-rail-hover)] disabled:cursor-not-allowed disabled:opacity-45";
    ...
    <>
        ...
        <RailPopover anchorRef={branchAnchorRef} width={300} ariaLabel={t("rightRail.gitActions.branchDialog")} onClose={() => setBranchOpen(false)}>
          <div className="p-3">
            <label className="flex h-9 items-center gap-2 rounded-[6px] border border-[var(--right-rail-divider)] px-3 text-[13px] text-[var(--right-rail-secondary)]">
              ...
              <input value={branchSearch} onChange={(event) => setBranchSearch(event.target.value)} className="min-w-0 flex-1 border-0 bg-transparent outline-none placeholder:text-[var(--right-rail-muted)]" placeholder={t("rightRail.gitActions.searchBranches")} autoFocus />
            </label>
          </div>
          ...
          <div className="max-h-56 overflow-y-auto px-2 pb-2">
            ...
            {filteredBranches.map((branch) => (
              <button key={branch.name} type="button" onClick={() => void checkoutBranch(branch.name)} disabled={branchBusy || branch.isCurrent} className="flex min-h-11 w-full items-center gap-2 rounded-[6px] px-2 text-left hover:bg-[var(--right-rail-hover)] disabled:opacity-100">
          ...
          <div className="border-t border-[var(--right-rail-divider)] p-3">
            <div className="flex gap-2">
              <input value={newBranchName} onChange={(event) => setNewBranchName(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") void createBranch(); }} className="h-9 min-w-0 flex-1 rounded-[6px] border border-[var(--right-rail-divider)] bg-transparent px-3 text-[13px] outline-none focus:border-[var(--right-rail-muted)]" placeholder={t("rightRail.gitActions.newBranchName")} />
              <button type="button" onClick={() => void createBranch()} disabled={!newBranchName.trim() || branchBusy} className="h-9 rounded-[6px] bg-[#202020] px-3 text-[12px] text-white disabled:opacity-35">{t("rightRail.gitActions.createAndCheckout")}</button>
            </div>
          </div>
        </RailPopover>
      ) : null}
      ...
      {commitOpen ? (
        <RailPopover anchorRef={commitAnchorRef} width={420} ariaLabel={t("rightRail.gitActions.commitDialog")} onClose={() => setCommitOpen(false)}>
          ...
          <div className="p-4">
            <textarea value={commitMessage} onChange={(event) => setCommitMessage(event.target.value)} className="h-20 w-full resize-none border-0 bg-transparent p-0 text-[14px] outline-none placeholder:text-[var(--right-rail-muted)]" placeholder={t("rightRail.gitActions.commitMessagePlaceholder")} onKeyDown={(event) => { if ((event.ctrlKey || event.metaKey) && event.key === "Enter") void commit(false); }} autoFocus />
