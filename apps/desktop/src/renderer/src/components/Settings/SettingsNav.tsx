...
function NavTabButton({
    ...
    return (
        <button
            ...
            onClick={onClick}
            className={`settings-pressable flex w-full items-start gap-3 rounded-xl px-3 py-3 text-left transition-[transform,background-color,color,box-shadow] duration-150 ease-out ${
                active
        ...
        >
            ...
            <span className="min-w-0">
                ...
                <span className={`mt-0.5 block text-[11px] leading-4 ${active ? 'text-[var(--mm-accent-blue)]/80' : 'text-[var(--mm-text-tertiary)]'}`}>
                    {tab.caption}
                </span>
            </span>
        </button>
...
function SearchResultButton({
    ...
    return (
        ...
        >
            ...
            <span className="min-w-0">
                <span className="block text-[13px] font-medium leading-5 text-[var(--mm-text-primary)]">{result.label}</span>
                <span className="mt-0.5 block text-[11px] leading-4 text-[var(--mm-text-tertiary)]">
                    {result.pageLabel} · {result.pageCaption}
                </span>
                {result.description && (
                    <span className="mt-1 block text-[11px] leading-4 text-[var(--mm-text-tertiary)]">
                        {result.description}
                    </span>
                )}
            </span>
...
export function SettingsNav({
    ...
    return (
        <aside className="flex w-[262px] shrink-0 flex-col border-r border-[var(--mm-border)] bg-[var(--mm-bg-sidebar)]">
            <div className="border-b border-[var(--mm-border)] px-4 py-4">
                <div className="flex items-center gap-2 rounded-xl border border-[var(--mm-border)] bg-[var(--mm-bg-main)] px-3 py-2.5">
                    <SearchIcon />
                    <input
                        type="search"
                        value={searchQuery}
            ...
            <nav className="min-h-0 flex-1 overflow-y-auto px-3 py-4" role="tablist" aria-label={t('settings.tabsAria')}>
                {inSearchMode ? (
                    ...
                    <div className="space-y-5">
                        {sections.map((section) => (
                            <section key={section.id}>
                                <div className="mb-2 px-2 text-[11px] font-medium uppercase tracking-[0.08em] text-[var(--mm-text-tertiary)]">
                                    {section.label}
                                </div>
